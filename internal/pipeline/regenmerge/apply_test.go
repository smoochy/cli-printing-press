...
// TestApplyPreservesNovelHelperRegistrations covers the extract+apply path
// for generator-emitted addNovelCommandIfAbsent wiring. Fresh root.go does
// not re-emit those calls; after --apply they must still be present.
func TestApplyPreservesNovelHelperRegistrations(t *testing.T) {
	t.Parallel()
...
func newNovelBackfillCmd(flags any) *cobra.Command {
	return &cobra.Command{Use: "backfill"}
}

func newNovelCoverageCmd(flags any) *cobra.Command {
	return &cobra.Command{Use: "coverage"}
}
`
	gomod := "module example.com/novel-helper\n\ngo 1.22\n"
	stagedPubDir, freshDir := stagedSyntheticPair(t,
		map[string]string{
			"go.mod":                 gomod,
			"internal/cli/root.go":   pubRoot,
	...
	require.NotEmpty(t, report.LostRegistrations,
		"helper-form novel registrations must appear in the lost set before apply")
	require.NoError(t, Apply(report, Options{Force: true}))

	got := readFileString(t, filepath.Join(stagedPubDir, "internal", "cli", "root.go"))
	assert.Contains(t, got, "newNovelBackfillCmd",
		"apply must re-inject addNovelCommandIfAbsent(newNovelBackfillCmd)")
	assert.Contains(t, got, "newNovelCoverageCmd",
		"apply must re-inject addNovelCommandIfAbsent(newNovelCoverageCmd)")
	assert.Contains(t, got, "addNovelCommandIfAbsent(rootCmd, newNovelBackfillCmd")
	assert.Contains(t, got, "addNovelCommandIfAbsent(rootCmd, newNovelCoverageCmd")
	assert.Contains(t, readFileString(t, filepath.Join(stagedPubDir, "internal", "cli", "novels.go")), "newNovelBackfillCmd",
		"novel implementation file must still be preserved")
}
